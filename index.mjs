...
const API_KEY =
  "live_nsoXb5hXeUZJ3zbjw8Et1Lh2HMT2OZ0Fg7spIlvzkvcbhJ7YW3E2Mldr5eq8ekaE";
...
async function loadCarousel() {
  ...
  const url = `/images/search?limit=25&breed_ids=${breedId}`;

  const response = await axios.get(url, {
    // Pass the updateProgress function to onDownloadProgress config option
...
const buildCarousel = (data, favourites) => {
  ...
  data.forEach((image) => {
    const img = Carousel.createCarouselItem(
      image.url,
      breedSelect.value,
      image.id
    );
  ...
  // Concatenate breed information into a single HTML string
  let infoHtml = "";
  data.forEach((image) => {
    const breed = image.breeds[0];
    if (breed) {
      infoHtml += `<p><strong>Description:</strong> ${breed.description}</p>`;
      infoHtml += `<p><strong>Temperament:</strong> ${breed.temperament}</p>`;
      infoHtml += `<p><strong>Origin:</strong> ${breed.origin}</p>`;
      infoHtml += `<p><strong>Wikipedia:</strong> <a href="${breed.wikipedia_url}" target="_blank">${breed.wikipedia_url}</a></p>`;
      infoHtml += `<p><strong>Life Span:</strong> ${breed.life_span}</p>`;
      infoHtml += `<p><strong>Weight:</strong> ${breed.weight.metric}</p>`;
    }
  });

  // Set infoDump.innerHTML once with the accumulated HTML
